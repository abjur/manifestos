...
Em teoria, a solução da API funciona muito bem. O problema é que falta articulação para implementar soluções padronizadas em todos os tribunais brasileiros. Por isso, faz sentido pensar numa solução centralizada que recebe e consolida os dados dos tribunais. Essa solução já existe. 

O Modelo de Interoperabilidade de Dados do Poder Judiciário (MNI) é um padrão genérico para processos judiciais. O padrão foi introduzido em 2010 pelo CNJ, com última atualização em 2014. O MNI contém todas as informações básicas de interesse dos processos judiciais, bem como partes e movimentações.

Adequar um relatório ao MNI é uma tarefa complicada. O único incentivo de um Tribunal de atender ao MNI seria para atender a critérios de boa governança de dados. Na prática, os Tribunais acabariam não enviando os dados em formato adequado.

O Selo Justiça em números é uma forma de corrigir esse problema. A partir de um sistema de pontuação que avalia a qualidade dos dados, o Selo cria um mecanismo de incentivo para que os Tribunais busquem uma boa governança de dados. Um Tribunal com selo Diamante, com nota mais alta, teria seu nome divulgado nas redes e teria um conjunto de benefícios do CNJ.

O Selo é um mecanismo eficiente para realizar uma integração contínua com os Tribunais. A análise dos dados enviados poderá indicar falhas nos sistemas dos Tribunais, que poderão ser corrigidos. Além disso, para obter maior qualidade dos dados, bastaria mudar a metodologia de pontuação do Selo.

Para que o Selo se torne a solução desejada, faltam apenas dois detalhes: i) API pública para acesso aos dados do Selo e ii) Criação de APIs para extração dos dados de cada tribunal. Sobre (i), bastaria montar uma API para acessar os dados do Selo, solução que acreditamos ser uma realidade em breve. Para (ii), uma alternativa seria incluir na pontuação do Selo a criação de uma API REST para acesso aos dados dos TJs na direção inversa. Posteriormente, os tribunais poderiam liberar essas APIs publicamente também, finalizando o processo de abertura de dados.
